...
# Rows containing these phrases will be ignored completely
SKIP_PHRASES = [
...
def parse_price_info(text):
    """
    Returns a dict: { 'std': float, 'bulk': float, 'thresh': int }
    Handles: "$2.00" OR "$2.00/each or $1.65/each for 6+"
    """
    info = {'std': 0.0, 'bulk': 0.0, 'thresh': 0}
    if not text: return info
    
    # 1. Check for Bulk Pattern: "$2... $1.65... 6+"
    # Regex looks for: $PRICE ... $PRICE ... NUMBER+
    bulk_match = re.search(r'\$(\d+\.?\d*).*?\$(\d+\.?\d*).*?(\d+)\+', text)
    if bulk_match:
        info['std'] = float(bulk_match.group(1))
        info['bulk'] = float(bulk_match.group(2))
        info['thresh'] = int(bulk_match.group(3))
        return info

    # 2. Standard Price Extraction
    # Look for $X.XX or just X.XX
    match = re.search(r'\$?(\d+\.\d{2})', text)
    if match:
        info['std'] = float(match.group(1))
        return info
        
    # 3. Fallback for integer prices ($5) if no dots found
    if '$' in text:
...
<style>
    ...
    /* ITEMS */
    .cg-item-row {{ display: flex; justify-content: space-between; align-items: start; padding: 15px 0; border-bottom: 1px solid #eee; }}
    ...
    .cg-price.unknown {{ color: #777; font-style: italic; font-weight: normal; font-size: 0.9em; }}
...
<script>
    ...
    // Store Item Data globally to handle bulk logic
    let cart = {{}}; 
    
    // 1. UPDATE QUANTITY (Handles Main List & Cart)
    function updateQty(name, price, bulkPrice, bulkThresh, rawPrice, change) {{
        // Init item if not exists
        if (!cart[name]) {{
            cart[name] = {{ qty: 0, price: price, bulkPrice: bulkPrice, bulkThresh: bulkThresh, raw: rawPrice }};
        }}
        ...
        if (cart[name].qty <= 0) {{
            delete cart[name];
            updateItemControls(name, 0);
        }} else {{
            updateItemControls(name, cart[name].qty);
        }}
    ...
    // 2. DOM MANIPULATION (Swaps + button for counters)
    function updateItemControls(name, qty) {{
        ...
        if (qty > 0) {{
            wrapper.innerHTML = `
                <button class="cg-qty-btn" onclick="triggerUpdate('${{name}}', -1)">-</button>
                <div class="cg-qty-val">${{qty}}</div>
                <button class="cg-qty-btn" onclick="triggerUpdate('${{name}}', 1)">+</button>
            `;
        }} else {{
            // Reset to Plus Button
            // We need to pass original args back. 
            // To do this simply, we reload the page? No. 
            // We kept the data attributes on the wrapper div for this reason.
            const p = wrapper.dataset.p; 
            const bp = wrapper.dataset.bp; 
            const bt = wrapper.dataset.bt; 
            const r = wrapper.dataset.r;
            wrapper.innerHTML = `<button class="cg-add-btn" onclick="updateQty('${{name}}', ${{p}}, ${{bp}}, ${{bt}}, '${{r}}', 1)">+</button>`;
        }}
    }}

    // Helper to call updateQty from the generated HTML inside wrapper
    // We need to look up the static data attributes from the wrapper parent
    window.triggerUpdate = function(name, change) {{
        const safeId = 'ctrl-' + name.replace(/[^a-zA-Z0-9]/g, "");
        const wrapper = document.getElementById(safeId);
        const p = parseFloat(wrapper.dataset.p); 
        const bp = parseFloat(wrapper.dataset.bp); 
        const bt = parseInt(wrapper.dataset.bt); 
        const r = wrapper.dataset.r;
        updateQty(name, p, bp, bt, r, change);
    }}
    ...
    // 3. PRICING ENGINE (Handles Bulk Logic & Integer Math)
    function calculateTotal() {{
        ...
        for (const [name, item] of Object.entries(cart)) {{
            ...
            if (item.bulkThresh > 0 && item.qty >= item.bulkThresh) {{
                effectivePrice = item.bulkPrice;
            }}
            
            // Math.round fixes floating point errors (10.59 * 100 = 1059)
            totalCents += Math.round(effectivePrice * 100) * item.qty;
        }}
    ...
    function updateUI() {{
        ...
        if (res.count > 0) bar.classList.add('visible');
    ...
    function renderCartItems() {{
        ...
        for (const [name, item] of Object.entries(cart)) {{
            ...
            if (item.bulkThresh > 0 && item.qty >= item.bulkThresh) {{
                priceDisplay = item.bulkPrice;
                note = `<span style="color:#27ae60; font-size:0.8em; margin-left:5px;">(Bulk applied!)</span>`;
            }} else if (item.bulkThresh > 0) {{
                note = `<span style="color:#e67c23; font-size:0.8em; margin-left:5px;">(Buy ${{item.bulkThresh}} for $${{item.bulkPrice}} ea)</span>`;
            }}

            container.innerHTML += `
                <div class="cg-cart-item">
                    <div class="cg-cart-name">
                        ${{name}} ${{note}}<br>
                        <span style="font-weight:normal; font-size:0.85em; color:#666;">@ $${{priceDisplay}}</span>
                    </div>
                    <div class="cg-cart-controls">
                        <button class="cg-qty-btn" onclick="triggerUpdate('${{name}}', -1)">-</button>
                        <span class="cg-qty">${{item.qty}}</span>
                        <button class="cg-qty-btn" onclick="triggerUpdate('${{name}}', 1)">+</button>
                    </div>
                </div>`;
        }}
    ...
    function sendEmail() {{
        ...
        for (const [name, item] of Object.entries(cart)) {{
            ...
            let label = "";
            if (item.bulkThresh > 0 && item.qty >= item.bulkThresh) {{
                finalPrice = item.bulkPrice;
                label = " (BULK)";
            }}
            
            body += `- [${{item.qty}}x] ${{name}} @ $${{finalPrice}}${{label}}\\n`;
        }}
    ...
    sections.forEach(s => {{
        ...
        a.onclick = (e) => {{
            e.preventDefault();
            document.querySelectorAll('.cg-nav a').forEach(l => l.classList.remove('active'));
...
if uploaded_file is not None:
    ...
    for row in reader:
        ...
        # 0. JUNK FILTER
        # If the row has a price, it's likely safe. If it has no price, check for junk keywords.
        line_text = " ".join(row).lower()
        if any(phrase in line_text for phrase in SKIP_PHRASES):
            continue

        # 1. Section Header (Must be ALL CAPS)
        ...
        # 2. Skip Table Headers (Rows that start with "Item")
        if row[0].lower().startswith("item"):
            continue
        ...
        if row[0]:
            ...
            # We store the raw data in the wrapper dataset so we can rebuild the button later
            body_content += f"""
